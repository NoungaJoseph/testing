            ...
            <div className="relative">
                ...
                <main className="flex-grow relative z-10">
                        ...
                        <div className="relative z-20 flex-1 flex flex-col justify-end max-w-7xl mx-auto px-6 lg:px-20 w-full pb-20 pt-40">
                            <FadeIn direction="up">
                                ...
                                <h1 className="text-white text-4xl md:text-6xl font-black leading-[0.9] tracking-tighter mb-8 max-w-4xl">
                                    {t('stories.hero.title')} <br />
                                    <span className="text-secondary ">{t('stories.hero.title_highlight')}</span>
                                </h1>
                    ...
                    <section className="py-24 px-6 lg:px-20 bg-transparent" id="stories">
                        <div className="max-w-7xl mx-auto">
                            <div className="flex flex-col items-start mb-16">
                                <span className="text-secondary font-black tracking-[0.4em] uppercase text-[10px] block mb-4">{t('stories.highlight.badge')}</span>
                                <h2 className="text-navy text-4xl md:text-5xl font-black tracking-tighter">{t('stories.highlight.title')} <span className="text-secondary ">{t('stories.highlight.title_highlight')}</span></h2>
                            </div>
                            <div className="relative group rounded-[3rem] overflow-hidden flex flex-col lg:flex-row transition-all duration-700 bg-navy">
                                ...
                                <div className="lg:w-1/2 p-12 lg:p-20 flex flex-col justify-center relative">
                                    ...
                                    <h3 className="text-4xl font-black text-white leading-tight tracking-tighter mb-8 ">{itemsT?.[0]?.title || stories[0].title}</h3>
                                    <p className="text-slate-400 text-lg font-medium leading-relaxed mb-10">{itemsT?.[0]?.excerpt || stories[0].excerpt}</p>
                                    <div className="pt-10 border-t border-white/10 flex items-center justify-between gap-4">
                                        <div>
                                            <p className="text-white text-xl font-black">{itemsT?.[0]?.author || stories[0].author}</p>
                                            <p className="text-secondary font-black text-[10px] uppercase tracking-[0.2em]">{itemsT?.[0]?.country || stories[0].country}</p>
                                        </div>
                    ...
                    <section className="py-24 px-6 lg:px-20 bg-transparent">
                        <div className="max-w-7xl mx-auto">
                            <div className="flex flex-col items-start mb-16">
                                <span className="text-slate-500 font-black tracking-[0.4em] uppercase text-[10px] block mb-4">{t('stories.latest.badge')}</span>
                                <h2 className="text-navy text-4xl md:text-5xl font-black tracking-tighter">{t('stories.latest.title')} <span className="text-secondary ">{t('stories.latest.title_highlight')}</span></h2>
                            </div>
                                    ...
                                    <FadeIn key={story.id} delay={i * 0.1} direction="up">
                                        <div className="rounded-[3rem] overflow-hidden transition-all duration-700 group flex flex-col h-full">
                                            ...
                                            <div className="p-10 flex-1 flex flex-col">
                                                <h3 className="text-navy text-2xl font-black mb-4 tracking-tight leading-tight group-hover:text-secondary transition-colors ">{ itemsT?.[i + 1]?.title || story.title }</h3>
                                                <p className="text-slate-500 font-medium leading-relaxed mb-10 flex-1">{itemsT?.[i + 1]?.excerpt || story.excerpt}</p>
                                                <div className="pt-8 border-t border-slate-50 flex items-center justify-between">
                                                    <div>
                                                        <p className="text-navy font-black text-sm uppercase tracking-tight">{itemsT?.[i + 1]?.author || story.author}</p>
                                                        <p className="text-secondary font-black text-[10px] uppercase tracking-[0.2em]">{itemsT?.[i + 1]?.country || story.country}</p>
                                                    </div>
                    ...
                    <section className="py-20 px-6 bg-navy text-center">
                        <div className="max-w-2xl mx-auto">
                            <h2 className="text-white text-3xl font-black mb-4">{t('stories.newsletter.title')}</h2>
                            <p className="text-slate-300 mb-8">{t('stories.newsletter.desc')}</p>
                            <div className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
                                <input
                                    type="email"
                                    placeholder={t('stories.newsletter.email_placeholder')}
                                ...
                                <button className="h-12 px-6 bg-secondary rounded-lg text-white font-bold hover:bg-[#00a8ae] transition-all">
                                    {t('stories.newsletter.btn_subscribe')}
                                </button>
                            </div>
                        </div>
                    </section>
